...
def fig_algorithm():
    ...
    try:
        ...
        r = requests.get(f"https://mermaid.ink/img/{b64}?type=png&bgColor=FFFFFF", timeout=30)
        r.raise_for_status()
        assert r.content[:8] == b"\x89PNG\r\n\x1a\n", "response was not a PNG"
        with open(f"{OUT}/fig_algorithm.png", "wb") as f:
            f.write(r.content)
    except Exception as e:  # noqa: BLE001 - keep any existing figure if the service is unreachable
        print(f"[figures] fig_algorithm skipped ({type(e).__name__}: {e}); keeping existing PNG")
